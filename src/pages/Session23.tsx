...
const Session23: React.FC = () => {
  ...
  const getCurrentHadiah = () => {
    if (hadiahIndex >= hadiahSesi3.length) return null;
    return hadiahSesi3[hadiahIndex];
  };
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      if (event.code === 'Enter') {
        ...
        // Next hadiah
        if (hadiahIndex < hadiahSesi3.length - 1 && winners.every(w => w)) {
          setHadiahIndex(hadiahIndex + 1);
        }
        // Selesai - set hadiahIndex ke nilai yang melebihi array length
        else if (hadiahIndex === hadiahSesi3.length - 1 && winners.every(w => w)) {
          setHadiahIndex(hadiahSesi3.length);
        }
      }
    };
  ...
  }, [hadiahIndex, winners, hadiahSesi3.length]);
        ...
        <div className="relative z-10 text-center">
          ...
          <div className="space-y-4">
            <button
              ...
              className="group relative px-8 py-4 text-xl bg-gradient-to-br from-yellow-400 to-orange-500 text-white rounded-2xl hover:from-yellow-500 hover:to-orange-600 transform hover:scale-105 transition-all duration-300 shadow-2xl hover:shadow-yellow-500/50 border border-yellow-400/20 font-bold"
            >
              ...
              <span className="relative z-10">🏆 Lanjut ke Grand Prize</span>
            </button>
  ...
  // Khusus untuk 4 pemenang menggunakan 2x2
  const gridCols = currentHadiah.jumlah === 4 ? 2 : (currentHadiah.jumlah <= 3 ? currentHadiah.jumlah : 3);
  const gridRows = currentHadiah.jumlah === 4 ? 2 : Math.ceil(currentHadiah.jumlah / 3);
  ...
  return (
    <div className="flex flex-col justify-center items-center h-screen bg-cover bg-center" style={{ backgroundImage: 'url(/bg.png)' }}>
      ...
      <div className="flex space-x-4 mt-10">
        {hadiahIndex === hadiahSesi3.length - 1 && winners.every(w => w) && (
          <button
            onClick={() => {
